...
functions_in_dir <- function(dir = "R", as_table = FALSE) {
  ...
  if (!length(files)) {
    return(if (as_table) data.frame(file = character(), function_name = character()) else setNames(list(), character()))
  }
  if (!as_table) {
...
find_unused_functions <- function(dir = "R") {
  ...
  tbl <- do.call(rbind, lapply(files, function(f) {
    ...
    data.frame(
      file = basename(f), function_name = fn, def_file = f,
      row.names = NULL, check.names = FALSE
    )
  }))
  if (is.null(tbl) || !nrow(tbl)) {
    return(data.frame(
      file = character(0), function_name = character(0),
                             ...
                             fill_namespace = TRUE) # set FALSE to skip namespace fallback
{
  ...
  if (nrow(edges)) {
    edges$file_src <- unname(name2file[edges$SOURCE])
    edges$file_src[is.na(edges$file_src)] <- "(unknown)"
    edges$file_tgt <- unname(name2file[edges$TARGET])
    edges$file_tgt[is.na(edges$file_tgt)] <- "(unknown)"
  } else {
    edges$file_src <- character(0)
    edges$file_tgt <- character(0)
  }

  # ---- per-function counts (flat) ----
  count_for <- function(tab, keys) {
    z <- as.integer(tab[keys])
    z[is.na(z)] <- 0L
    z
  }
  ...
  if (nrow(edges)) {
    ...
    if (length(key_tbl)) {
      ...
      file_edges <- data.frame(
        file_src = pr[, 1], file_tgt = pr[, 2],
      ...
      file_edges <- data.frame(
        file_src = character(0), file_tgt = character(0),
