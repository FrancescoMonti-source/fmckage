...
make_group_table <- function(data,
    ...
    ordered_groups <- c(intersect(grp_chr_order, have_groups),
    ...
    if (isTRUE(include_missing)) {
        ...
        miss_fmt <- function(m, t) ifelse(is.na(m) | is.na(t), NA_character_,
        ...
        for (g in ordered_groups) {
            mc <- paste0("miss_", g); tc <- paste0("total_", g); lbl <- label_of(g)
